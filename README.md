...
TxConcert.sln
├── src/
│   ├── TxConcert.Api/              # ASP.NET Core Web API entry point
...
│   │   └── appsettings.json
│   │
│   ├── TxConcert.Application/      # Use cases, CQRS handlers
│   │   ├── Common/
...
│   │
│   ├── TxConcert.Domain/           # Pure domain layer
│   │   ├── Common/
...
│   │
│   ├── TxConcert.Infrastructure/   # Interface implementations
│   │   ├── Persistence/
...
│   │   └── DependencyInjection.cs
│   │
│   └── TxConcert.Worker/           # Background job host
│       ├── Program.cs                   # Quartz scheduler with 4 recurring jobs
│       └── Dockerfile
│
├── test/
│   ├── TxConcert.UnitTests/        # Domain + Application unit tests
│   └── TxConcert.IntegrationTests/ # WebApplicationFactory + Testcontainers
│
...
### 0. Install EF Core CLI Tool

```bash
dotnet tool install --global dotnet-ef
```

If the tool is already installed but not on your PATH (macOS/Linux):
...
This starts:
- **PostgreSQL** on port `5432` (user: `postgres`, password: `postgres`, db: `tx_concert`)
- **Redis** on port `6379`
...
```bash
dotnet ef database update --project src/TxConcert.Infrastructure --startup-project src/TxConcert.Api
```

To create a new migration after model changes:

```bash
dotnet ef migrations add <MigrationName> --project src/TxConcert.Infrastructure --startup-project src/TxConcert.Api
```

> **Note:** You may see a `HostAbortedException` log during migration commands — this is expected and harmless. EF Core briefly starts the app to discover the DbContext, then aborts it.

### 3. Run the API

```bash
dotnet run --project src/TxConcert.Api
```

The API starts on `http://localhost:5555` (Development profile). Swagger UI is available at `/swagger`.

### 4. Run the Worker (Optional)

```bash
dotnet run --project src/TxConcert.Worker
```
...
```bash
dotnet test test/TxConcert.UnitTests
```
...
```bash
dotnet test test/TxConcert.IntegrationTests
```
...
```bash
export ConnectionStrings__DefaultConnection="Host=localhost;Port=5432;Database=tx_concert_test;Username=postgres;Password=postgres"
```
